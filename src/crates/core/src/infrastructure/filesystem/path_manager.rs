...
impl PathManager {
    ...
    pub async fn initialize_project_directories(&self, workspace_path: &Path) -> BitFunResult<()> {
        ...
        debug!("Project-level directories initialized for {:?}", workspace_path);
        Ok(())
    }
...
pub fn get_path_manager_arc() -> Arc<PathManager> {
    try_get_path_manager_arc().expect("Failed to create global PathManager")
}
