...
impl FileTreeService {
    ...
    pub async fn build_tree(&self, root_path: &str) -> Result<Vec<FileTreeNode>, String> {
        ...
        self.build_tree_recursive(&root_path_buf, &root_path_buf, &mut visited, 0).await
    }

    pub async fn build_tree_with_stats(&self, root_path: &str) -> BitFunResult<(Vec<FileTreeNode>, FileTreeStatistics)> {
        let root_path_buf = PathBuf::from(root_path);
        ...
        let nodes = self.build_tree_recursive_with_stats(
            &root_path_buf, 
            &root_path_buf, 
            &mut visited, 
            0, 
        ...
        for entry in entries {
            ...
            if is_directory {
                if !is_symlink || self.options.follow_symlinks {
                    match self.build_tree_recursive(&entry_path, root_path, visited, depth + 1).await {
                        Ok(children) => {
        ...
        for entry in entries {
            ...
            if let Some(file_size) = size {
                ...
                if file_size > 10 * 1024 * 1024 {
                    stats.large_files.push((entry_path.to_string_lossy().to_string(), file_size));
                }
            }
            ...
            if is_directory {
                if !is_symlink || self.options.follow_symlinks {
                    match self.build_tree_recursive_with_stats(
                        &entry_path, 
                        root_path, 
                        visited, 
                        depth + 1, 
    ...
    fn should_skip_file(&self, file_name: &str) -> bool {
        ...
        if !self.options.include_hidden && file_name.starts_with('.') && file_name != ".gitignore" && file_name != ".bitfun" {
            return true;
        }
    ...
    pub async fn get_directory_contents(&self, path: &str) -> Result<Vec<FileTreeNode>, String> {
        ...
        while let Some(entry) = read_dir.next_entry().await
            ...
            let entry_path = entry.path();
            let is_directory = entry.file_type().await
                .map(|t| t.is_dir())
                .unwrap_or(false);
        ...
        walker.run(|| {
            ...
            Box::new(move |entry| {
                ...
                if let Some(file_name) = path.file_name() {
                    let file_name_str = file_name.to_string_lossy();
                    if Self::should_skip_file_static(&file_name_str) || Self::is_binary_file_static(&file_name_str) {
                        return ignore::WalkState::Continue;
                    }
                }
                ...
                if search_content {
                    let results_len = results.lock().unwrap().len();
