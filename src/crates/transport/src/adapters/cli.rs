...
impl TransportAdapter for CliTransportAdapter {
    async fn emit_event(&self, _session_id: &str, event: AgenticEvent) -> anyhow::Result<()> {
        let cli_event = match event {
            ...
            AgenticEvent::DialogTurnCompleted { session_id, turn_id, .. } => {
                CliEvent::DialogTurnCompleted { session_id, turn_id }
            }
            _ => return Ok(()),
        };
        
        self.tx.send(cli_event).map_err(|e| {
            anyhow::anyhow!("Failed to send CLI event: {}", e)
        })?;
    ...
    async fn emit_text_chunk(&self, _session_id: &str, chunk: TextChunk) -> anyhow::Result<()> {
        self.tx.send(CliEvent::TextChunk(chunk)).map_err(|e| {
            anyhow::anyhow!("Failed to send text chunk: {}", e)
        })?;
        Ok(())
    }
    
    async fn emit_tool_event(&self, _session_id: &str, event: ToolEventPayload) -> anyhow::Result<()> {
        self.tx.send(CliEvent::ToolEvent(event)).map_err(|e| {
            anyhow::anyhow!("Failed to send tool event: {}", e)
        })?;
        Ok(())
    }
    
    async fn emit_stream_start(&self, session_id: &str, turn_id: &str, round_id: &str) -> anyhow::Result<()> {
        self.tx.send(CliEvent::StreamStart {
            session_id: session_id.to_string(),
            turn_id: turn_id.to_string(),
            round_id: round_id.to_string(),
        }).map_err(|e| {
            anyhow::anyhow!("Failed to send stream start: {}", e)
        })?;
        Ok(())
    }
    
    async fn emit_stream_end(&self, session_id: &str, turn_id: &str, round_id: &str) -> anyhow::Result<()> {
        self.tx.send(CliEvent::StreamEnd {
            session_id: session_id.to_string(),
            turn_id: turn_id.to_string(),
            round_id: round_id.to_string(),
        }).map_err(|e| {
            anyhow::anyhow!("Failed to send stream end: {}", e)
        })?;
        Ok(())
    }
    
    async fn emit_generic(&self, event_name: &str, payload: serde_json::Value) -> anyhow::Result<()> {
        self.tx.send(CliEvent::Generic {
            event_name: event_name.to_string(),
            payload,
        }).map_err(|e| {
            anyhow::anyhow!("Failed to send generic event: {}", e)
        })?;
        Ok(())
    }
