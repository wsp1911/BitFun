...
use super::string_utils::{truncate_str, prettify_result};

pub fn render_tool_card<'a>(
    ...
    match tool_call.tool_name.as_str() {
        ...
        "write_file" | "write_file_tool" | "search_replace" => render_write_file_card(&mut items, tool_call, theme),
...
fn render_read_file_card<'a>(
    ...
    // Get file path
    let file_path = tool_call.parameters.get("file_path")
    ...
    let (status_icon, status_style) = match &tool_call.status {
        ToolCallStatus::Running | ToolCallStatus::Streaming => ("*", theme.style(StyleKind::Primary)),
...
fn render_codebase_search_card<'a>(
    ...
    let query = tool_call.parameters.get("query")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown");
...
fn render_grep_card<'a>(
    ...
    let pattern = tool_call.parameters.get("pattern")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown");
...
fn render_list_dir_card<'a>(
    ...
    let path = tool_call.parameters.get("target_directory")
...
fn extract_key_params(params: &serde_json::Value) -> String {
    if let Some(obj) = params.as_object() {
        let priority_keys = ["path", "file_path", "target_file", "query", "pattern", "command", "message"];
